import { ApiProperty } from '@nestjs/swagger';
import { Role } from '../entities/role.entity';

export class RoleListItemDto {
  @ApiProperty({ description: '역할 ID', example: 1 })
  roleId: number;

  @ApiProperty({ description: '역할 이름', example: 'ADMIN' })
  roleName: string;

  @ApiProperty({ description: '역할 표시명', example: '관리자', nullable: true })
  displayName: string | null;

  @ApiProperty({ description: '역할 설명', example: '시스템 관리자 역할', nullable: true })
  description: string | null;

  @ApiProperty({ description: '활성 상태 (1: 활성, 0: 비활성)', example: 1 })
  isActive: number;

  @ApiProperty({ description: '생성일시', example: '2026-02-17T12:00:00.000Z' })
  createdAt: Date;

  @ApiProperty({ description: '수정일시', example: '2026-02-17T12:00:00.000Z' })
  updatedAt: Date;

  @ApiProperty({ description: '테넌트 ID', example: 1 })
  tenantId: number;

  @ApiProperty({ description: '해당 역할을 가진 사용자 수', example: 5 })
  userCount: number;

  @ApiProperty({ description: '해당 역할에 할당된 권한 수', example: 25 })
  permissionCount: number;
}

export class PageInfoDto {
  @ApiProperty({ description: '현재 페이지', example: 1 })
  currentPage: number;

  @ApiProperty({ description: '페이지당 항목 수', example: 20 })
  pageSize: number;

  @ApiProperty({ description: '전체 항목 수', example: 45 })
  totalItems: number;

  @ApiProperty({ description: '전체 페이지 수', example: 3 })
  totalPages: number;
}

export class FindRolesResponseDto {
  @ApiProperty({ description: '역할 목록', type: [RoleListItemDto] })
  items: RoleListItemDto[];

  @ApiProperty({ description: '페이지네이션 정보', type: PageInfoDto })
  pageInfo: PageInfoDto;
}
