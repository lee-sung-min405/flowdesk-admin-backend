...
export class AuthController {
  ...
  @ApiBody({ description: '리프레시 토큰 요청', type: RefreshRequestDto })
  ...
  @ApiBody({ description: '폐기할 리프레시 토큰을 전송', type: LogoutDto })
  @UsePipes(new ValidationPipe({ whitelist: true }))
  ...
  async logoutAll(@Req() req: any) {
    ...
    await this.authService.revokeAllRefreshTokens(userSeq);
    return { ok: true };
  }
